...
class FastShipping_Correios_Model_Http_Client_Adapter_Socket
    extends Zend_Http_Client_Adapter_Socket
{
    ...
    const CACHE_TYPE = 'fastshipping_correios';
    ...
    public function connect($host, $port = 80, $secure = false)
    {
        if (Mage::app()->useCache(self::CACHE_TYPE)) {
            ...
            if (!($mode == FastShipping_Correios_Model_Source_CacheMode::MODE_CACHE_ONLY)) {
                try {
    ...
    public function read()
    {
        ...
        if (Mage::app()->useCache(self::CACHE_TYPE)) {
            ...
            if ($cacheMode == FastShipping_Correios_Model_Source_CacheMode::MODE_HTTP_PRIOR) {
                try {
            ...
            } elseif ($cacheMode == FastShipping_Correios_Model_Source_CacheMode::MODE_CACHE_PRIOR) {
            ...
            } elseif ($cacheMode == FastShipping_Correios_Model_Source_CacheMode::MODE_CACHE_ONLY) {
                $response = $cache->load();
            }
        } else {
            $response = parent::read();
     ...
     *
     * @return FastShipping_Correios_Model_Cache
     */
